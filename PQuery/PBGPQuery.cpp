//
// Created by yyh on 2023/4/25.
//

#include "BGPQuery.h"

/***
 * New function for gpstore, written by ???
 * 
*/
...
bool BGPQueryNew::CheckIthEdgePredicate(unsigned int ith_edge) {
  ...
  auto edge_var_descrip = dynamic_pointer_cast<EdgeVarDescriptor>(get_vardescrip_by_id(e_id_[ith_edge]));
  return edge_var_descrip->is_predicate_var;
}

const shared_ptr<VarDescriptor>& BGPQueryNew::get_vardescrip_by_id(unsigned id){
  assert(false && "get_vardescrip_by_id is not implemented yet!");
}

const shared_ptr<VarDescriptor> & BGPQueryNew::get_vardescrip_by_index(unsigned index){
  assert(false && "get_vardescrip_by_index is not implemented yet!");
}

bool BGPQueryNew::is_var_satellite_by_id(unsigned id){
  assert(false && "is_var_satellite_by_id is not implemented yet!");
}

bool BGPQueryNew::is_var_satellite_by_index(unsigned index){
  assert(false && "is_var_satellite_by_index is not implemented yet!");
}

unsigned BGPQueryNew::get_var_id_by_index(unsigned index){
  assert(false && "get_var_id_by_index is not implemented yet!");
}

unsigned BGPQueryNew::get_triple_num(){
  assert(false && "get_triple_num is not implemented yet!");
}

unsigned BGPQueryNew::get_total_var_num(){
  assert(false && "get_total_var_num is not implemented yet!");
}

//todo: this function is not reasonal, for sparql query, one so_var may has many edges with the same predicate
bool BGPQueryNew::get_so_var_edge_direct_type(unsigned var_id, unsigned edge_id){
  assert(false && "get_total_var_num is not implemented yet!");
}
...
/***
 * Functions for new PBGPQuery[.h|cpp], used for Cypher, which are written by Chunshan Zhao.
*/


/***
 * Below are old BGPQuery[.h|cpp], which are written by Linglin Yang.
 * 
*/
/*=============================================================================
