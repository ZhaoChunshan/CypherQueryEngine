...
public:
	// cannot descriminate literal with entity(i.e. node)
	// note: NotDecided only used in PlanGenerator
    // Assume eid == pid for RDF data.
	enum class VarType{Node, Edge, NotDecided};     // rdf,pg
    ...
    enum class EdgeDirectType{LeftArrow, RightArrow, Undirected };    //pg
    
    enum class ItemType{SubType, PreType, ObjType}; // dead code
    ...
    vector<ItemType> so_var_item_type;      // dead code

    /* adjacent Edge Node information of this Node*/
	// only used when var_type_ == Node
    // Adjacent Edges
	vector<unsigned> so_edge_index_;        // 第i条边在BGP三元组中的位置
	vector<char> so_edge_direct_type_;      // see Util::EDGE_[OUT|IN]
    vector<EdgeType> so_edge_var_const_type_;// 第i边常量还是变量
    vector<unsigned> so_edge_eid_;          // Adjacent Edge, VarID(varialbe) or Eid(constant)
    // Neighbor nodes
    vector<NodeType> so_edge_nei_type_;     // 指定变量还是常量
	vector<unsigned> so_edge_nei_;          // 变量，为varid,常量，为KV id
    ...
    /* adjacent Node information of this Edge */
	// pre_var edge info, only used when var_type == Edge
    ...
    VarDescriptor(unsigned id, VarType var_type, const string &var_name, const std::vector<unsigned>& labels_or_types, const std::map<unsigned, Value>& props_, const std::vector<Expression*>& exps);

    // Constructor for PG Edge Var
    VarDescriptor(unsigned id, VarType var_type, const string &var_name, EdgeDirectType edge_direct, int range_left, int range_right, const std::vector<unsigned>& labels_or_types, const std::map<unsigned, Value>& props_, const std::vector<Expression*>& exps);
    ...
    */
	void update_so_var_edge_info(unsigned edge_nei_id, unsigned edge_id, char edge_direct_type,
...
class BGPQuery {
public:
    enum class BGPMode {Homomorphism, NodeIsomorphism, EdgeIsomorphism};    // rdf, pg
...
//	maybe not use join_pre_var_num
	...
	shared_ptr<map<unsigned,shared_ptr<IDList>>> var_candidates_cache;
    ...
    */
	...
	void ScanAllVarByBigBGPID(BGPQuery *big_bgpquery, const vector<string>& _query_var);    // rdf only
    ...
    void setBGPMode(BGPMode bgp_mode);
